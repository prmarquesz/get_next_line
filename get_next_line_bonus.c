...
/*   Updated: 2021/06/24 02:20:51 by proberto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || fd > OPEN_MAX || BUFFER_SIZE <= 0 || line == 0)
		return (-1);
	res = ft_is_there_a_line_break(storage[fd]);
	if (res >= 0)
	{
		res = ft_stoline(line, &storage[fd], res);
		if (res == -1)
			*line = NULL;
		return (res);
	}
	...
	res = ft_get_buffer(fd, line, &buffer, &storage[fd]);
	if (res < 0)
		*line = NULL;
	free(buffer);
	return (res);
